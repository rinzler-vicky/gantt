...
export default class Bar {
	...
	draw_bar() {
		const barColor = this.task.color || '#fff'; // Use task color or a default color
		const importantColor = this.task.importantColor || '#94c4f4'; // Use task color or a default color
		...
		this.$bar = createSVG('rect', {
			...
			class:
				...
				(/^((?!chrome|android).)*safari/i.test(navigator.userAgent) &&
					...
					? ' safari'
					: ''),
			fill: this.task.important ? importantColor : barColor, // Set the fill color
	...
	draw_progress_bar() {
		if (this.invalid) return;
		const progressColor = this.task.progressColor || (this.task.important ? '#2c94ec' : '#ebeef0'); // Use task color or a default color
